  ...
  if ($products_new_split->number_of_rows > 0) {
    ...
    while (!$products_new->EOF) { 
      ...
      if (PRODUCT_NEW_LIST_WEIGHT != '0' and zen_get_show_product_switch($products_new->fields['products_id'], 'weight')) {
        $display_products_weight = '<span class="weight">' . '<strong>'.TEXT_PRODUCTS_WEIGHT.'</strong>' . $products_new->fields['products_weight'] . TEXT_SHIPPING_WEIGHT . str_repeat(substr(PRODUCT_NEW_LIST_WEIGHT, 3, 1)).'</span>';
      } else {
      ...
      if (PRODUCT_NEW_LIST_MANUFACTURER != '0' and zen_get_show_product_switch($products_new->fields['products_id'], 'manufacturer')) {
        $display_products_manufacturers_name = '<span class="brand">'.($products_new->fields['manufacturers_name'] != '' ? '<strong>'.TEXT_MANUFACTURER.'</strong>' . ' ' . $products_new->fields['manufacturers_name'] . str_repeat(substr(PRODUCT_NEW_LIST_MANUFACTURER, 3, 1)) : '').'</span>';
      } else {
